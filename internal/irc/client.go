...
type IRCClient struct {
	...
	registrationApplied   chan struct{}              // Closed after the current registration's queued callbacks have been applied (guarded by mu)
	registrationApplyOnce *sync.Once                 // Guards registrationApplied for duplicate 376/422 replies (guarded by mu)
...
func (c *IRCClient) onConnect(e ircmsg.Message) {
	// Connect waits for this callback to finish before reporting success to the
	// app. Keeping the signal in a defer also releases the waiter if a future
	// handler change returns early or panics (the dispatcher contains panics).
	defer c.finishRegistrationCallbacks()
...
func (c *IRCClient) setupHandlers() {
	...
	// Register the lifecycle callback last. irc-go invokes callbacks for a line
	// in registration order, so this is enqueued behind every other 376/422
	// application handler. Connect can then use its completion as a precise
	// barrier for the whole registration transcript without blocking the socket
	// reader while that transcript is applied.
...
func (c *IRCClient) Connect() error {
	...
	// Prepare all per-registration state before the socket can deliver 001/376.
	// In particular, callbacks run asynchronously now, so resetting this state
	// after conn.Connect returned would race the already-queued handlers.
	registrationApplied := c.beginRegistrationCallbacks()
	...
	// irc-go's handshake returns as soon as its protocol-level 376/422 handler
	// runs. Cascade callbacks for the registration transcript are deliberately
	// asynchronous and may still be queued at that point. Wait for the final
	// lifecycle callback so callers cannot publish app/plugin events that
	// overtake earlier IRC events (or place a success marker before the MOTD).
	<-registrationApplied
	c.mu.RLock()
	registrationSucceeded := c.connected && !c.abandoned
	c.mu.RUnlock()
	if !registrationSucceeded {
		return fmt.Errorf("failed to connect: connection closed while registration callbacks were being applied")
	}
...
// beginRegistrationCallbacks creates the completion boundary for one explicit
// Connect call. It also re-arms auto-join before any registration callbacks can
// observe it; doing either operation after conn.Connect returns races the
// asynchronous callback worker.
func (c *IRCClient) beginRegistrationCallbacks() <-chan struct{} {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.registrationApplied = make(chan struct{})
	c.registrationApplyOnce = &sync.Once{}
	c.autoJoinOnce = &sync.Once{}
	return c.registrationApplied
}

// finishRegistrationCallbacks releases Connect after onConnect and every
// earlier callback for the registration transcript have finished. Some broken
// servers can send more than one MOTD terminator, so closing is idempotent.
func (c *IRCClient) finishRegistrationCallbacks() {
	c.mu.RLock()
	done := c.registrationApplied
	once := c.registrationApplyOnce
	c.mu.RUnlock()
