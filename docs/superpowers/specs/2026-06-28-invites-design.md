...
3. **Volume-based collapse:** a sender's group is shown **expanded as individual channel rows when small (< 5 channels)** and **auto-collapsed into a single summary row when large (≥ 5 channels)**, e.g. "stranger123 invited you to 8 channels," with the one-click "Ignore sender" right there. Collapse triggers on **volume alone, never on trust** — so a friend enthusiastically inviting a newcomer to several channels behaves exactly like anyone else: a short list shows normally; only a genuine flood collapses, and even then nothing is hidden, just tidied. *Frontend, presentation.*
4. **Safety caps (backstops):** keep at most ~20 channels per sender (surplus dropped with a "+N more" affordance) and an overall per-network cap on senders. These are high enough that legitimate use never hits them. *Backend.*
...
- **Session-only, in-memory, backend-owned.** `handleInvite` stops writing actionable (target-is-me) invites to the status buffer; instead it records them in a per-network, mutex-guarded in-memory store on `App`, applies dedup/caps/block, tags the `trusted` flag, and emits an event. The frontend renders the node from a bound getter plus live events (same pattern as PM conversations and monitor presence). Restart clears all invites and the block set. No schema/sqlc change.
...
- **Go unit tests:** trust classification (buddy / non-buddy / blocked); dedup by (sender, channel); per-sender and per-network caps; the block set; third-party `invite-notify` stays a status line; send-result numeric → feedback mapping.
...
- Collapse = **by sender, volume-triggered (≥ 5), non-destructive.**
